...
class TransformerPolicy8NoSharedRelpos(nn.Module):
    def __init__(self, hps, obs_config):
        super(TransformerPolicy8NoSharedRelpos, self).__init__()
        ...
        self.version = 'transformer_v8_no_shared_relpos'
        ...
        if hps.ally_enemy_same:
            self.item_nets.append(PosItemBlock(
                obs_config.dstride(),
                hps.d_item, hps.d_item // 2 * hps.dff_ratio, norm_fn, hps.item_ff,
            ...
            if self.nally > 0:
                self.item_nets.append(PosItemBlock(
                    obs_config.dstride(), hps.d_item, hps.d_item // 2 * hps.dff_ratio, norm_fn, hps.item_ff,
                    mask_feature=7,  # Feature 7 is hitpoints
        ...
        if hps.nmineral > 0:
            self.item_nets.append(PosItemBlock(
                obs_config.mstride(), hps.d_item, hps.d_item // 2 * hps.dff_ratio, norm_fn, hps.item_ff,
        ...
        if hps.ntile > 0:
            self.item_nets.append(PosItemBlock(
                obs_config.tstride(), hps.d_item, hps.d_item // 2 * hps.dff_ratio, norm_fn, hps.item_ff,
    ...
    def latents(self, x, action_masks):
        ...
        for item_net in self.item_nets:
            emb, mask = item_net(x, origin, direction, active_agents.batch_index)
            emb_list.append(emb)
        ...
        print('emb_list', [emb.size() for emb in emb_list])
        print('mask_list', [mask.size() for mask in mask_list])
        items = torch.cat(emb_list, dim=1)
...
class PosItemBlock(nn.Module):
    def __init__(self,
        ...
        self.embedding = InputEmbedding(d_in+3, d_model, norm_fn)
    ...
    def forward(self, x, origin=None, direction=None, indices=None, privileged=False):
        if privileged and self.start_privileged:
        ...
        if origin is not None:
            select = (x[:, :, self.mask_feature] != 0)[indices]
            pos = x[indices, :, 0:2]
            ...
            norm_relpos = torch.cat([direction, torch.sqrt(dist.unsqueeze(-1))], dim=2)
            norm_relpos_sparse = norm_relpos[select]
            x_sparse = x[indices][select]
            x_sparse = torch.cat([norm_relpos_sparse, x_sparse], dim=1)
            ...
            mask = select == False
            x_sparse = x[select]
            x_sparse = torch.cat([torch.zeros(size=(x_sparse.size(0), 3,), device=x.device), x_sparse], dim=1)
            active = SparseSequence.from_mask(select)
        ...
        if x_sparse.numel() > 0:
            ...
            return torch.cuda.FloatTensor(mask.size()[0], mask.size()[1], self.d_model).fill_(0), mask
        else:
            return torch.zeros(mask.size()[0], mask.size()[1], self.d_model), mask
